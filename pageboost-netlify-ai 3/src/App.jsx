...
function FacebookLoginButton({ onLoginSuccess, onLogout }) {
  ...
  const handleLoginSuccess = (authResponse) => {
    ...
    window.FB.api(
      '/me/accounts',
      { fields: 'id,name,access_token,permissions,picture' },
      (response) => {
        if (response && !response.error) {
          console.log('📄 Pages:', response.data)
          const pages = response.data
          setUserData({ userId: userID, accessToken, pages })
          if (onLoginSuccess) {
            onLoginSuccess({ userId: userID, accessToken, pages })
          }
        } else {
...
{ icon: '💬', title: 'Ask Questions',       desc: 'Questions in captions in crease comments by 162%. End every post with one.' },
...
{ id: 2, type: 'text',  content: '💡 The secret to succes s isn't working harder — it's working smarter. What's one intentional thing you did today? Tell us below! 👇 #MondayMotivation', scheduled: 'Today, 1:00 PM', status: 'scheduled', e ngagement_pred: 'Very High' },
{ id: 3, type: 'photo', content: '📸 Behind the scenes — how we craft our products with love and care. Every detail matters because YOU matter. ❤️ #B ehindTheScenes #Quality', scheduled: 'Today, 7:00 PM', status: 'scheduled', engagement_pred: 'High' },
...
const css =  `@import url('https://fonts.googleapis.com/css2?family=Syne:wght@400;600;700;800` `&family=DM+Sans:wght@300;400;500;600` `&display=swap'); *{box-sizing:border-box;margin:0;padding:0} :root{--bg:#0a0d14;--surface:#111520;--card:#161c2e;--border:#1f2a45;--blue:#1877f2;--blue-g:rgba(24,119,242,.22);--acc` `ent:#00d4ff;--green:#00e676;--gold:#ffd740;--red:#ff5252;--purple:#b388ff;--text:#e8edf5;--muted:#6b7a99;--ff:'Syne',sans-serif;--fb:'DM Sans',sans-serif} body{background:var(--bg)` `;color:var(--text);font-family:var(--fb)} .app{min-height:100vh;background:var(--bg);background-image:radial-gradient(ellipse at 20% 0%,rgba(24,119,242,.08) 0%,transparent 55%),rad` `ial-gradient(ellipse at 80% 100%,rgba(0,212,255,.05) 0%,transparent 55%)} .hdr{display:flex;align-items:center;justify-content:space-between;padding:18px 32px;background:rgba(17,21` `,32,.96);border-bottom:1px solid var(--border);backdrop-filter:blur(20px);position:sticky;top:0;z-index:100} .logo{display:flex;align-items:center;gap:12px} .logo-icon{width:40px;h` `eight:40px;border-radius:10px;background:linear-gradient(135deg,#1877f2,#0a52c4);display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:900;color:#fff;fo` `nt-family:var(--ff);box-shadow:0 0 24px var(--blue-g)} .logo-name{font-family:var(--ff);font-size:18px;font-weight:800;letter-spacing:-.3px} .logo-sub{font-size:11px;color:var(--mu` `ted);margin-top:-2px} .hdr-r{display:flex;align-items:center;gap:10px} .live{display:flex;align-items:center;gap:6px;padding:5px 13px;border-radius:20px;background:rgba(0,230,118,.` `1);border:1px solid rgba(0,230,118,.3);font-size:12px;color:var(--green);font-weight:600} .live-dot{width:7px;height:7px;border-radius:50%;background:var(--green);animation:pulse 1` `.5s infinite} .fb-badge{display:flex;align-items:center;gap:6px;padding:5px 13px;border-radius:20px;font-size:12px;font-weight:600;border:1px solid;cursor:pointer;transition:all .2` `s} .fb-badge.on{background:rgba(24,119,242,.15);border-color:rgba(24,119,242,.4);color:#60a5fa} .fb-badge.off{background:rgba(255,82,82,.1);border-color:rgba(255,82,82,.3);color:va` `r(--red)} @keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.5;transform:scale(1.3)}} .nav{display:flex;gap:2px;padding:0 32px;border-bottom:1px solid var(--border)` `;background:var(--surface);overflow-x:auto} .nav::-webkit-scrollbar{display:none} .tab{padding:14px 18px;font-size:13px;font-weight:600;color:var(--muted);border:none;background:no` `ne;cursor:pointer;border-bottom:2px solid transparent;transition:all .2s;white-space:nowrap;font-family:var(--fb)} .tab:hover{color:var(--text)} .tab.on{color:var(--blue);border-bo` `ttom-color:var(--blue)} .main{padding:28px 32px;max-width:1400px;margin:0 auto} .col{display:flex;flex-direction:column;gap:20px} .g2{display:grid;grid-template-columns:1fr 1fr;gap` `:20px} .g3{display:grid;grid-template-columns:repeat(3,1fr);gap:20px} .g4{display:grid;grid-template-columns:repeat(4,1fr);gap:16px} .card{background:var(--card);border:1px solid v` `ar(--border);border-radius:16px;padding:24px;transition:border-color .2s,box-shadow .2s} .card:hover{border-color:rgba(24,119,242,.3);box-shadow:0 0 28px rgba(24,119,242,.05)} .car` `d-title{font-family:var(--ff);font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--muted);margin-bottom:18px} .stat{background:var(--card);border:` `1px solid var(--border);border-radius:16px;padding:20px;position:relative;overflow:hidden} .stat::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:lin` `ear-gradient(90deg,var(--blue),var(--accent))} .stat-label{font-size:12px;color:var(--muted);font-weight:600;text-transform:uppercase;letter-spacing:.8px} .stat-val{font-family:var` `(--ff);font-size:32px;font-weight:800;margin:6px 0 4px;letter-spacing:-1px} .stat-ch{font-size:12px;font-weight:600;color:var(--green)} .btn{padding:10px 20px;border-radius:10px;fo` `nt-size:13px;font-weight:600;cursor:pointer;border:none;font-family:var(--fb);transition:all .2s;display:inline-flex;align-items:center;gap:7px} .btn-p{background:var(--blue);color` `:#fff} .btn-p:hover:not(:disabled){background:#1565d8;box-shadow:0 0 20px var(--blue-g);transform:translateY(-1px)} .btn-p:disabled{opacity:.5;cursor:not-allowed} .btn-g{background` `:rgba(255,255,255,.05);color:var(--text);border:1px solid var(--border)} .btn-g:hover{background:rgba(255,255,255,.09)} .btn-d{background:rgba(255,82,82,.15);color:var(--red);borde` `r:1px solid rgba(255,82,82,.3)} .btn-s{background:rgba(0,230,118,.15);color:var(--green);border:1px solid rgba(0,230,118,.3)} .btn-fb{background:var(--blue);color:#fff;width:100%;j` `ustify-content:center;padding:14px 20px;font-size:14px;border-radius:12px} .btn-fb:hover:not(:disabled){background:#1565d8;box-shadow:0 0 24px var(--blue-g);transform:translateY(-1` `px)} .btn-fb:disabled{opacity:.5;cursor:not-allowed} .inp,.sel,.ta{width:100%;padding:11px 14px;border-radius:10px;background:rgba(255,255,255,.04);border:1px solid var(--border);c` `olor:var(--text);font-family:var(--fb);font-size:14px;outline:none;transition:border-color .2s,box-shadow .2s} .inp:focus,.sel:focus,.ta:focus{border-color:var(--blue);box-shadow:0` ` 0 0 3px var(--blue-g)} .ta{resize:vertical;min-height:130px;line-height:1.6} .sel option{background:var(--card)} .frow{display:flex;gap:12px} .frow` `>*{flex:1} label{font-size:12px;color:var(--muted);font-weight:600;margin-bottom:6px;display:block;text-transform:uppercase;letter-spacing:.5px} .post-item{background:rgba(255,255,` `255,.03);border:1px solid var(--border);border-radius:12px;padding:16px;display:flex;gap:14px;align-items:flex-start;transition:all .2s} .post-item:hover{border-color:rgba(24,119,2` `42,.3);background:rgba(24,119,242,.04)} .ptype{width:38px;height:38px;border-radius:10px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:18px} .pcont` `ent{font-size:13px;line-height:1.6;color:var(--text);margin-bottom:10px} .pmeta{display:flex;align-items:center;gap:8px;flex-wrap:wrap} .tag{padding:3px 10px;border-radius:20px;fon` `t-size:11px;font-weight:600} .t-blue{background:rgba(24,119,242,.15);color:#60a5fa;border:1px solid rgba(24,119,242,.2)} .t-green{background:rgba(0,230,118,.15);color:var(--green);` `border:1px solid rgba(0,230,118,.2)} .t-gold{background:rgba(255,215,64,.15);color:var(--gold);border:1px solid rgba(255,215,64,.2)} .t-red{background:rgba(255,82,82,.15);color:var` `(--red);border:1px solid rgba(255,82,82,.2)} .t-purple{background:rgba(179,136,255,.15);color:var(--purple);border:1px solid rgba(179,136,255,.2)} .ai-box{background:rgba(24,119,24` `2,.06);border:1px solid rgba(24,119,242,.2);border-radius:12px;padding:16px;font-size:13px;line-height:1.8;white-space:pre-wrap} .err-box{background:rgba(255,82,82,.08);border:1px ` `solid rgba(255,82,82,.25);border-radius:12px;padding:14px;font-size:13px;color:var(--red)} .ok-box{background:rgba(0,230,118,.08);border:1px solid rgba(0,230,118,.25);border-radius` `:12px;padding:14px;font-size:13px;color:var(--green);line-height:1.6} .info-box{background:rgba(255,215,64,.06);border:1px solid rgba(255,215,64,.2);border-radius:12px;padding:16px` `;font-size:13px;line-height:1.7} .dot-anim::after{content:'...';animation:dots 1.4s infinite} @keyframes dots{0%{content:'.'}33%{content:'..'}66%{content:'...'}} .step{display:flex` `;gap:16px;align-items:flex-start;padding:18px;background:rgba(255,255,255,.02);border:1px solid var(--border);border-radius:12px;margin-bottom:12px} .step-num{width:34px;height:34p` `x;border-radius:50%;background:var(--blue);color:#fff;font-family:var(--ff);font-weight:800;font-size:15px;display:flex;align-items:center;justify-content:center;flex-shrink:0;box-` `shadow:0 0 16px var(--blue-g)} .step-title{font-family:var(--ff);font-size:15px;font-weight:700;margin-bottom:6px} .step-desc{font-size:13px;color:var(--muted);line-height:1.7} .st` `ep-desc a{color:#60a5fa;text-decoration:none} .step-desc a:hover{text-decoration:underline} .step-code{background:rgba(0,0,0,.5);border:1px solid var(--border);border-radius:8px;pa` `dding:10px 14px;font-family:monospace;font-size:12px;color:var(--accent);margin-top:8px;word-break:break-all;line-height:1.6} .fb-page-card{display:flex;align-items:center;gap:16px` `;padding:20px;background:rgba(24,119,242,.08);border:1px solid rgba(24,119,242,.25);border-radius:14px} .fb-avatar{width:60px;height:60px;border-radius:50%;background:linear-gradie` `nt(135deg,#1877f2,#0a52c4);display:flex;align-items:center;justify-content:center;font-size:26px;flex-shrink:0;overflow:hidden;box-shadow:0 0 20px var(--blue-g)} .fb-avatar img{wid` `th:100%;height:100%;object-fit:cover} .fb-name{font-family:var(--ff);font-size:20px;font-weight:800} .fb-fans{font-size:13px;color:var(--muted);margin-top:3px} .trend{display:flex;` `align-items:center;padding:12px 0;border-bottom:1px solid var(--border)} .trend:last-child{border-bottom:none} .t-bar{height:3px;border-radius:2px;background:rgba(255,255,255,.06);` `margin-top:6px;width:100%} .t-fill{height:100%;border-radius:2px;background:linear-gradient(90deg,var(--blue),var(--accent))} .tgrid{display:grid;grid-template-columns:repeat(7,1fr` `);gap:8px} .tcol{display:flex;flex-direction:column;align-items:center;gap:5px} .tday{font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase} .tslot{padding:5px` ` 6px;border-radius:7px;font-size:10px;font-weight:600;background:rgba(24,119,242,.12);border:1px solid rgba(24,119,242,.2);color:#60a5fa;text-align:center;width:100%} .tslot.best{b` `ackground:rgba(0,230,118,.15);border-color:rgba(0,230,118,.3);color:var(--green)} .ebar{height:4px;width:100%;border-radius:2px;background:rgba(255,255,255,.06);margin-top:3px} .ef` `ill{height:100%;border-radius:2px;background:linear-gradient(90deg,var(--green),#69f0ae)} .strat{background:rgba(255,255,255,.02);border:1px solid var(--border);border-radius:12px;` `padding:18px;transition:all .2s} .strat:hover{border-color:rgba(24,119,242,.3);transform:translateY(-2px)} .prog-wrap{display:flex;align-items:center;gap:12px;margin-bottom:12px} .` `prog-bar{flex:1;height:6px;background:rgba(255,255,255,.06);border-radius:3px;overflow:hidden} .prog-fill{height:100%;border-radius:3px} .prog-label{font-size:12px;color:var(--mute` `d);min-width:88px} .prog-val{font-size:12px;font-weight:700;min-width:34px;text-align:right} .cgrid{display:grid;grid-template-columns:repeat(7,1fr);gap:4px} .chead{text-align:cent` `er;font-size:11px;font-weight:700;color:var(--muted);padding:6px 0;text-transform:uppercase} .ccell{aspect-ratio:1;border-radius:8px;display:flex;flex-direction:column;align-items:` `center;justify-content:flex-start;padding:6px 4px;font-size:11px;font-weight:600;color:var(--muted);cursor:pointer;transition:all .15s;background:rgba(255,255,255,.02);border:1px s` `olid transparent} .ccell:hover{background:rgba(24,119,242,.1);border-color:rgba(24,119,242,.2);color:var(--text)} .ccell.has{background:rgba(24,119,242,.1);border-color:rgba(24,119` `,242,.25);color:var(--text)} .ccell.today{background:var(--blue);color:#fff;border-color:var(--blue)} .cdot{width:4px;height:4px;border-radius:50%;background:var(--accent);margin-t` `op:2px} .shdr{display:flex;align-items:center;justify-content:space-between;margin-bottom:20px} .stitle{font-family:var(--ff);font-size:20px;font-weight:800} .ssub{font-size:13px;c` `olor:var(--muted);margin-top:3px} .toast{position:fixed;bottom:24px;right:24px;padding:14px 20px;border-radius:12px;font-size:13px;font-weight:600;z-index:9999;animation:slideIn .3` `s ease;max-width:340px;line-height:1.5;cursor:pointer} .toast.success{background:#00e676;color:#000} .toast.error{background:var(--red);color:#fff} @keyframes slideIn{from{transfor` `m:translateY(20px);opacity:0}to{transform:translateY(0);opacity:1}} @media(max-width:900px){.main{padding:16px}.g2,.g3,.g4{grid-template-columns:1fr}.tgrid{grid-template-columns:re` `peat(4,1fr).hdr{padding:14px 16px}.nav{padding:0 16px}.frow{flex-direction:column}}`
...
function FacebookTab({ fbStatus, posts, onToast, fbLoginData, onFbLogin }) {
...
if (!fbStatus.connected && !fbLoginData) return <FacebookSetup />

// Show Facebook Login if not connected via env vars
if (!fbStatus.connected && fbLoginData) {
  const page = fbLoginData.pages?.[0]
  return (
    <div className="col">
      <div className="card">
        <div className="card-title">📘 Facebook Login</div>
        <div style={{ marginBottom: 20 }}>
          <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 16, lineHeight: 1.7 }}>
            Connect your Facebook account to manage and post to your pages directly from PageBoost AI.
          </p>
          <FacebookLoginButton 
            onLoginSuccess={onFbLogin}
            onLogout={() => onFbLogin(null)}
          />
        </div>
        ...
        {page && (
          <div className="fb-page-card">
            ...
            <div style={{ flex: 1 }}>
              ...
              <div className="fb-fans">Page connected successfully</div>
            </div>
      ...
      <div className="card">
        ...
        <div className="col" style={{ gap: 14 }}>
          <div>
            ...
            <textarea 
              className="ta" 
              placeholder="Write your post here — or generate one in the ✨ Create tab and paste it here..." 
              value={content} 
          ...
          <div>
            ...
            <input 
              className="inp" 
              placeholder="https://example.com/photo.jpg — leave blank for text-only post" 
              value={imageUrl} 
          ...
          {lastPost && (
            <div className="ok-box">
              🎉 Post is live! <a href={lastPost.postUrl} target="_blank" rel="noreferrer" style={{ color: 'var(--green)', fontWeight: 700 }}>
                View on Facebook ↗
              </a>
            </div>
          )}
      ...
      <div className="card">
        ...
        <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 14 }}>
          Click "Use this post" to load it into the publisher above.
        </p>
...
// Original FacebookTab content for env var connection
...
const daysInMonth=new Date(today.getFullYear(),today.ge tMonth()+1,0).getDate()
...
const handleFbLogin = (data) => {
  setFbLoginData(data)
  if (data) {
    showToast('✅ Connected to Facebook!', 'success')
  }
}
return (
 < >
 ...
 <div className= "hdr-r " >
 <div className={ `fb-badge ${fbStatus.connected || fbLoginData ? 'on' : 'off'}` } onClick={() = > setTab('facebook')} >
{fbStatus.connected || fbLoginData ? `✓ ${fbStatus.page?.name || fbLoginData?.pages?.[0]?.name || 'Facebook Connected'}` : '⚠ Connect Facebook →'}
 </div >
     ...
     <main className= "main " >
      {tab==='dashboard'  & & (
         <div className= "col " >
          ...
          {!fbStatus.connected && !fbLoginData  & & (
          ...
          {(fbStatus.connected || fbLoginData)  & & (
             <div className= "ok-box " >
              🎉 Connected to  <strong >{fbStatus.page?.name || fbLoginData?.pages?.[0]?.name} </strong > · {fbStatus.page?.fans?.toLocaleString() || 'Page connected'} followers ·{' '}
      ...
      {tab==='facebook'   & &  <FacebookTab fbStatus={fbStatus} posts={posts} onToast={showToast} fbLoginData={fbLoginData} onFbLogin={handleFbLogin} / >}
      {tab==='create'     & & (
         <div className= "col " >
